...
def HSV_distance(data,model):
    ...
    return np.sqrt( ( np.sin(h1)*s1*v1 - np.sin(h2)*s2*v2 )**2  + ( np.cos(h1)*s1*v1 - np.cos(h2)*s2*v2 )**2 + ( v1 - v2 )**2  )
...
def get_catalog_match(RGBcolor_predicted, df, category,):
    ...
    for i in category_rows['index']:
        if category_rows.at[i,'percent1'] > 0.5:
            ...
            category_rows.at[i,'rgb_dist_1'] = HSV_distance((r1, g1, b1) , RGBcolor_predicted)
        elif category_rows.at[i,'percent2'] > 0.5:
            rgb2 = category_rows.at[i,'rgb2']
            r2,g2,b2 = rgb2[0], rgb2[1], rgb2[2]
            category_rows.at[i,'rgb_dist_2'] = HSV_distance((r2, g2, b2) , RGBcolor_predicted)
...
class outfit_input:
    ...
    def write_image(self, save_name='temp_0.jpg'):
        self.save_name= save_name
        save_index = 0
        while os.path.exists('static/'+self.save_name)==True:
            save_index+=1
            self.save_name = 'temp_'+str(save_index)+'.jpg'
        self.imgobj.save( 'static/'+self.save_name )
    ...
    def crop_torso(self):
        ...
        response = client.face_detection(image = load_image_GCV('static/'+self.save_name))
        ...
        try:
            ...
            (x0,y0), height, width= (upper_left_vertex, height, width) #vertices
    ...
    def predict_color_matches(self, model_vec):
        ...
        self.color_match_0 = [  model_0.predict(self.features_linear_diff[0].reshape(1,6) )[0] , \
                                model_1.predict(self.features_linear_diff[0].reshape(1,6) )[0] , \
                                model_2.predict(self.features_linear_diff[0].reshape(1,6) )[0] ]
                                #model_1.predict(self.features_linear_diff[0].reshape(-1,1))[0] , \
                                #model_2.predict(self.features_linear_diff[0].reshape(-1,1))[0] ]

        self.color_match_1 = [model_0.predict(self.features_linear_diff[1].reshape(1,6))[0] , \
                                model_1.predict(self.features_linear_diff[1].reshape(1,6))[0] , \
                                model_2.predict(self.features_linear_diff[1].reshape(1,6))[0] ]
        '''
