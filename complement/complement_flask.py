...
def submission():
    ...
    input_img_file = 'temp.jpg'
    input_outfit.write_image(save_path=input_img_file)
    ...
    print('\n', np.shape(input_outfit.keras_input0), '\n')
    ...
    if predictor_set=='rgb':
        ...
        temp_model_vec = pickle.load(open('models/regression_model_linearDiff_HSVpredictors.p', 'rb'))
        model_S, model_V = temp_model_vec[1], temp_model_vec[2]
    ...
    html_urls = zip(return_product_urls, return_img_urls)
    return render_template("index.html", input_img_file = input_img_file, product_urls=html_urls)
